(function($) {
	...
	$.pagerComponent = {
			...
			showContent: function(page, $table) {      //顯示輸入頁次的內容
				...
				for(var i = startRec; i < endRec; i++)
				{
					...
					for(var j in datas[i])
					{
						...
						{
							if(datas[i][j] == "N")
								content += "<td nowrap><div id='updownSale' val='N'><input type='radio' name='onsale" + dataId + "' id='Y" + dataId + "' style='width:50px' /><label for='Y" + dataId + "'>上架</label><input type='radio' name='onsale" + dataId + "' id='N" + dataId + "' style='width:50px' checked /><label for='N" + dataId + "'>下架</label></div></td>";								
							else
								content += "<td nowrap><div id='updownSale' val='Y'><input type='radio' name='onsale" + dataId + "' id='Y" + dataId + "' style='width:50px' checked /><label for='Y" + dataId + "'>上架</label><input type='radio' name='onsale" + dataId + "' id='N" + dataId + "' style='width:50px' /><label for='N" + dataId + "'>下架</label></div></td>";							
						}
						else
				...
				$("tbody", $table).find("tr").each(function() {
					var $thisTR = $(this);
					var dataId = $thisTR.find("td:eq(1)").text();
					var $btnDiv = $thisTR.find("td:last").find("div#updownSale");
					
					$btnDiv.find("input[name='onsale" + dataId + "']").click(function() {
						var btnVal = $(this).attr("id").substr(0, 1);
						var defaultVal = $btnDiv.attr("val");
						if(defaultVal !== btnVal)
							$.pagerComponent.EditForm.updownSale($table, $btnDiv, dataId, btnVal);
					});
				});
				...
				$.ajax({
					...
					success: function(data) {
						if(data)
						{
							...
							var $details = $("<tr id='detailsTR'><td colspan='" + ColuNum + "' align='left' style='background-color:#D3F0EE;'><div id='details' style='color:#666666; width:96%; height:45px; padding:10px; border:1px solid #CCCCCC; text-align:left;'></div></td></tr>");
							var detailContent = "成本價格：" + data[0].prod_cost + "　　銷售價格：" + data[0].prod_slprice + "　　安全存量：" + data[0].prod_save + "　　期初庫存：" + data[0].prod_iqty + "　　期末庫存：" + data[0].prod_nqty + "<br /><br /><a href='#' id='hideDetails' style='margin-left:90%; margin-right:0; padding:0; color:#0099CC;'>隱藏</a><span class='detailClose'></span><br />";							
		...
		EditForm: {
			updownSale: function($table, $btnDiv, dataId, YorN) {
				var Opts = $.pagerComponent.options[0];
				var pageChange = $("#pageList").find("a.active").text();
				$.ajax({
					type: "GET",
					url: Opts.loaddata_page,
					data: "type=UDSALETYPE&id=" + dataId + "&onsale=" + YorN + "&sdate=" + Opts.sDate + "&edate=" + Opts.eDate,
					dataType: "json",
					success: function() {
						alert("資料已送出!");
		...
		Filter: {
			...
			getDatasByType: function($obj) {
				...
				var type = $obj.val();
				var allDatas = $.pagerComponent.Datas.dataStore;
				//取得欄位值(eg. pID為欄位值)為key的值:一般取值的用法為allDatas[i].pID,但是現在key值(即pID)為變數,所以以 [key值] 來取代 .pID 的用法
				for(var i = 0; i < allDatas.length; i++)
				{
