(function($) {
	...
	$.pagerComponent = {
		...
		lib: {
			...
			showContent: function(page, $table) {      //顯示輸入頁次的內容
				...
				{
					content += "<tr><td nowrap><a href='#' class='detailLink'>" + Opts.detailstxt + "</a></td>";
					for(var j in datas[i])
					{
						var txtAlign = "";
						if(j === "warning")
							txtAlign = "style='text-align:center;'";
						
						if(datas[i][j] === "Y")
							content += "<td class='ui-state-error' " + txtAlign + "><font color='white'>" + datas[i][j] + "</font></td>";
						else
							content += "<td " + txtAlign + ">" + datas[i][j] + "</td>";
					}
					content += "</tr>";
				}
				...
				$("a.detailLink", $table).each(function() {
					$(this).click(function() {
						...
						$.pagerComponent.Details.showInfo($table, $(this), dataId);
		...
		Details: {
			showInfo: function($table, $link, dataId) {
				...
				$.ajax({
					...
					success: function(data) {
						if(data)
						{
							...
							var $details = $("<tr id='detailsTR'><td colspan='" + ColuNum + "' align='left' style='background-color:#99CCCC;'><div id='details' style='color:#333333; width:96%; height:50px; padding:10px; border:1px solid #99CCFF; text-align:left;'></div></td></tr>");
							var detailContent = "安全存量：" + data[0].prod_save + "　　期初庫存：" + data[0].prod_iqty + "　　現有庫存：" + data[0].prod_nqty + "<input type='button' id='updQty' style='margin-left:290px;' value='更改' /><br /><br /><a href='#' id='hideDetails' style='margin-left:90%; margin-right:0; padding:0; color:#0099CC;'>隱藏</a><span class='detailClose'></span><br />";							
							$details.hide().insertAfter($thisTR);
							$("div#details").html(detailContent);
							$details.show();
							
							$("input#updQty").button().click(function() {
								$.pagerComponent.EditForm.loadForm($table, $thisTR.find("td:eq(1)").text(), $thisTR.find("td:eq(2)").text(), data[0].prod_iqty, data[0].prod_nqty );
							});
		...
		EditForm: {
			loadForm: function($table, dataId, pname, iqty, nqty) {
				var $pName = $("span#prodname"), $Iqty = $("input#Iqty"), $Nqty = $("input#Nqty");
				var allFields = $([]).add($Iqty).add($Nqty);
				var $tips = $("p.validateTips");
				
				$pName.html("<font color='#0099CC'>" + pname + "</font>");
				$Iqty.spinner().spinner("value", iqty);
				$Nqty.spinner().spinner("value", nqty);
				...
				var $dialog = $("#dialog").dialog({
					...
					buttons: {
						"修改": function() {
							var pageChange = $("#pageList").find("a.active").text();
							allFields.removeClass("ui-state-error");
							
							var isValid = true;	
							isValid = isValid && checkValue($Iqty, "[ 期初庫存 ]");
							isValid = isValid && checkValue($Nqty, "[ 期末庫存 ]");
							isValid = isValid && checkNumber($Iqty, /[0-9]/g, "[ 期初庫存 ]");
							isValid = isValid && checkNumber($Nqty, /[0-9]/g, "[ 期初庫存 ]");
							
							if(isValid)
							{
								$.ajax({
									type: "GET",
									url: $.pagerComponent.options[0].loaddata_page,
									data: "type=UDSTOCK&id=" + dataId + "&iqty=" + $Iqty.spinner("value") + "&nqty=" + $Nqty.spinner("value"),
									dataType: "json",
									cache: false,
									success: function(data) {
										if(data)
					...
					close: function() {
						...
						allFields.val(0).removeClass("ui-state-error");
					}
				});
			}
		},
		...
		Datas: {
			...
			loadData: function($table, action) {
				var Opts = $.pagerComponent.options[0];
				var sql = "type=WARNLIST&sdate=" + Opts.sDate + "&edate=" + Opts.eDate;
				...
				$.ajax({
					...
					error: function(textStatus, errorThrown) {
						...
						alert("存取錯誤：" + textStatus + " - " + errorThrown);
					}
				});
			},
			setDatas: function() {
	...
	function checkValue($obj, objName)
	{
		if($obj.val().length <= 0 || $obj.val() < 0)
		{
			...
			tips(objName + "欄位值不可空白或小於0，請重新輸入!");
			return false;
		}
		else
			return true;
