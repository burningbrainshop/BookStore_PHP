	...
	$.pagerComponent = {
			...
			showContent: function(page, $table) {      //顯示輸入頁次的內容
				...
				for(var i = startRec; i < endRec; i++)
				{
					...
					for(var j in datas[i])
					{
						...
						if(j === "prod_photoname") 
						{
							var flag = (datas[i][j]) ? "background-color:#5DDEC4;" : "";
							content += "<td><img id='picLoad' src='images/hard-drive-download.png' width='20px' height='20px' style='padding:1px; border:1px solid #33ACEF; cursor:hand; " + flag + "' alt='" + datas[i][j] + "' /></td>";
						}
						else
				...
				//為每個[圖檔] 連結設定編輯事件
				$("img#picLoad", $table).each(function() {
					$(this).click(function() {
						var $obj = $(this);
						var dataId = $(this).closest("tr").find("td:eq(1)").text();
						
						$("#fileUpload").remove();
						
						var blockUI = $("<div id='bgBlock' />");  //遮罩層
						var $fp;
						if($obj.attr("alt").indexOf(".") != -1)
						{
							$fp = $("<div id='fileUpload' style='width:260px; height:150px; z-index:99998; border:1px solid #3399CC; padding:5px; background-color:#99CCFF;'><font color='#0066CC'>- 圖檔 / 圖片  <a href='#' id='reloadfile'>重新上傳</a> -</font><span class='pictureClose' style='z-index:99999;'></span><hr style='height:1px; color:#AAAAAA;'><img id='bookImg' style='z-index:100000; cursor:hand;' src='showImg.php?photoName=pictures/" + $obj.attr("alt") + "' /></div>");							
						}
						else
						{
							$fp = $("<div id='fileUpload' style='width:260px; height:150px; z-index:99998; border:1px solid #3399CC; padding:5px; background-color:#99CCFF;'><font color='#0066CC'>- 圖檔 / 圖片 <a href='#' id='reloadfile'>重新上傳</a> -</font><span class='pictureClose' style='z-index:99999;'></span><hr style='height:1px; color:#AAAAAA;'><iframe id='uploadFrame' name='uploadFrame' frameBorder='0' scrolling='no' width='250px' height='120px' src='upload.php?id=" + dataId + "'></iframe></div>");
						}
						...
						$(blockUI)   
							.animate({
							  "opacity": "0.4"     //改變遮罩的透明度
								...
								var pos = $obj.offset();
								$fp.appendTo("body").css({ "position": "absolute", "left": pos.left, "top": pos.top + 24 }).show();
								$("span.pictureClose").click(function() {
									$fp.remove();
									$(blockUI).remove();
									$.pagerComponent.Datas.loadData($table, page);
								});
								
								$("a#reloadfile").click(function() {
									var pos = $obj.offset();
									$("div#fileUpload").remove();
									$fp = $("<div id='fileUpload' style='width:260px; height:150px; z-index:99998; border:1px solid #3399CC; padding:5px; background-color:#99CCFF;'><font color='#0066CC'>- 圖檔 / 圖片 <a href='#' id='reloadfile'>重新上傳</a> -</font><span class='pictureClose' style='z-index:99999;'></span><hr style='height:1px; color:#AAAAAA;'><iframe id='uploadFrame' name='uploadFrame' frameBorder='0' scrolling='no' width='250px' height='120px' src='upload.php?id=" + dataId + "'></iframe></div>");
				...
				$.ajax({
					...
					success: function(data) {
						if(data)
						{
							...
							var $details = $("<tr id='detailsTR'><td colspan='" + ColuNum + "' align='left' style='background-color:#D3F0EE;'><div id='details' style='color:#666666; width:96%; height:55px; padding:10px; border:1px solid #CCCCCC; text-align:left;'></div></td></tr>");
							var detailContent = "成本價格：" + data[0].prod_cost + "　　銷售價格：" + data[0].prod_slprice + "　　安全存量：" + data[0].prod_save + "　　期初庫存：" + data[0].prod_iqty + "　　期末庫存：" + data[0].prod_nqty + "<br /><br />上 / 下架：" + data[0].prod_onsale + "　　　建檔日期：" + data[0].prod_astablish + "<br /><a href='#' id='hideDetails' style='margin-left:90%; margin-right:0; padding:0; color:#0099CC;'>隱藏</a><span class='detailClose'></span><br />";							
		...
		EditForm: {
			loadForm: function(op, $table) {
				var $pName = $("input#prodname"), $prodNo = $("select#prod_code"), $vendNo = $("select#vend_no"),
					  $saveNum = $("input#savenum"), $cPrice = $("input#cprice"), $sPrice = $("input#sprice"), $pDesp = $("textarea#desp");

				var inputFields = $([]).add($pName).add($saveNum).add($cPrice).add($sPrice).add($pDesp);
					...
					$.ajax({
						...
						success: function(data) {
							if(data)
							{
								...
								$prodNo.val(data[0].prod_code);
								$vendNo.val(data[0].vend_no);
								$saveNum.val(data[0].prod_save);
								$cPrice.val(data[0].prod_cost);
								$sPrice.val(data[0].prod_slprice);
								$pDesp.val(data[0].prod_desp);
							}
						},
						error: function(data, textStatus) {
							alert("存取錯誤:" + textStatus + " - " + data);
				...
				var $dialog = $("#dialog").dialog({
					...
					buttons: {
						"確定": function() {
							...
							isValid = isValid && checkEmpty($pName, "[ 產品名稱 ]");
							isValid = isValid && checkEmpty($saveNum, "[ 安全存量 ]");
							isValid = isValid && checkEmpty($cPrice, "[ 成本價格 ]");
							isValid = isValid && checkEmpty($sPrice, "[ 銷售價格 ]");
							
							if(isValid == true)
							{
								if(op === "NEW")
									var inputData = { type: "NEW", prod_name: $pName.val(), prod_code: $prodNo.val(), vend_no: $vendNo.val(), savenum: $saveNum.val(), cprice: $cPrice.val(), sprice: $sPrice.val(), desp: $pDesp.val(), sdate: Opts.sDate, edate: Opts.eDate  };
								else
									var inputData = { type: "SAVE", id: op, prod_name: $pName.val(), prod_code: $prodNo.val(), vend_no: $vendNo.val(), savenum: $saveNum.val(), cprice: $cPrice.val(), sprice: $sPrice.val(), desp: $pDesp.val(), sdate: Opts.sDate, edate: Opts.eDate };
		...
		Sort: {
			...
			initialize: function($table) {
				...
				$thElement.each(function(index) {
					...
					if(index > 0 && index < $thElement.size()-1)
