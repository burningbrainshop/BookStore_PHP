(function($) {
	$.fn.purchasegrid = function(Opts) {
		...
		return this.each(function() {			
			...
			$.pagerComponent.options[0].sDate = settings.sDate;
			$.pagerComponent.options[0].eDate = settings.eDate;
			
			//初始化新增按鈕
			$("table#newTB input#newBtn").button().click(function(event) {
				event.preventDefault();
				$.pagerComponent.EditForm.loadForm("NEW", $table);
			});
	...
	$.pagerComponent = {
		...
		lib: {
			...
			showContent: function(page, $table) {      //顯示輸入頁次的內容
				...
				{
					content += "<tr><td nowrap><a href='#' class='editLink'>" + Opts.edittext + "</a> <a href='#' class='deleteLink'>" + Opts.deletetext + "</a> <a href='#' class='detailLink'>" + Opts.detailstext + "</a></td>";
				...
				$("a.editLink", $table).each(function() {
					$(this).click(function() {
						...
						$.pagerComponent.EditForm.loadForm(dataId, $table);
		...
		Details: {
				...
				$.ajax({
					...
					success: function(data) {
						if(data)
						{
							...
							var $details = $("<tr id='detailsTR'><td colspan='" + ColuNum + "' align='left' style='background-color:#99CCCC;'><div id='details' style='color:#333333; width:96%; height:65px; padding:10px; border:1px solid #99CCFF; text-align:left;'></div></td></tr>");
							var detailContent = "<table id='purchDetailsList' border='0' width='100%'><tr><td colspan='2' style='border:0; border-bottom:1px dashed;'>採購單　產品項目:<button id='detailInfo' style='margin-left:460px;'>編輯</button></td></tr>";
							for(var i in data)
							{
								detailContent += "<tr><td>" + data[i].prod_name + "</td><td>" + data[i].purch_num + "</td></tr>";
							}
							detailContent += "</table><br /><a href='#' id='hideDetails' style='margin-left:90%; margin-right:0; padding:0; color:#0099CC;'>隱藏</a><span class='detailClose'></span><br />";							
							$details.hide().insertAfter($thisTR);
							$("div#details").html(detailContent);
							$details.show();
							
							$("a#hideDetails").click(function() { $("tr#detailsTR").empty(); });
							$("#detailInfo").button().click(function() { $.pagerComponent.Details.editInfo($link, dataId); });
						}
					},
			...
			editInfo: function($llink, dataId) {
				var infoContent = "<table id='DetailList'><tr><td colspan='2' align='left' style='color:#0066CC; background-color:#669999; font-size:14px;'>編輯　採購產品資料</td></tr>";
				var prodType = "<option value=''></option>";
				var prodList;
				$.ajax({
					type: "GET",
					url: "product_set.php",
					data: "type=LISTNOTNULL",
					dataType: "json",
					success: function(data) {
						if(data)
						{
							for(var i in data)
							{
								prodType += "<option value=" + data[i].Id + ">" + data[i].Name + "</option>";
							}
						}
					},
					error: function(data, textStatus) {
				...
				$.ajax({
					type: "GET",
					url: "purchase_info.php",
					data: "type=ONE&id=" + dataId,
					dataType: "json",
					cache: false,
					success: function(data) {
						if(data)
						{
							infoContent += "<tr><td style='text-align:right;'>採購單編號</td><td style='text-align:left;'>" + data[0].purch_no + "</td></tr>";
							infoContent += "<tr><td style='text-align:right;'>供應商名稱</td><td style='text-align:left;'>" + data[0].vend_name + "</td></tr>";
							infoContent += "<tr><td style='text-align:right;'>　交貨日期</td><td style='text-align:left;'>" + data[0].purch_date + "</td></tr>";
							infoContent += "<tr><td colspan='2' height='75%' width='100%' valign='top'>";
							infoContent += "<table id='newPurchDetails' align='center' style='border:0; width:90%; height:100%;'>";
							infoContent += "<tr><td style='border:0;'><fieldset><legend style='color:#999999; padding:3px;'>產品資料</legend><font size='2' color='#0099CC'>產品類別：</font><select id='prodType' name='prodType'>" + prodType + "</select><br /><select id='prodList' name='prodList'></select><br /><font size='2' color='#0099CC'>產品數量：</font><input type='text' id='prodNum' name='prodNum' style='width:30px;' />&nbsp;&nbsp;<input type='button' id='newPurchProd' value='新增' /></td></tr></fieldset></tr>";
							infoContent += "<tr><td style='border:0;' height='40%'><div id='productCart' style='width:450px; height:200px; overflow:scroll; border:1px solid #CCCCCC;'></div></td></tr>";
							infoContent += "</table>";
							infoContent += "</td></tr>";
							infoContent += "<tr><td colspan='2' style='text-align:right;'><input type='button' id='insertPD' value='確定' />&nbsp;<input type='button' id='cancelPD' value='取消' style='margin-right:30px;' /></td></tr>";
							infoContent += "</table>";
							
							$("<div id='purchDetail' style='padding: 5px; background-color:#FFF;'></div>").appendTo("body").hide().html(infoContent);							
							$("div#productCart").droppable({
								drop: function(event, ui) {
									var pcPos = $("div#productCart").offset();
									$("div#eachProduct").each(function() {
										$(this).css({ "top" : (pcPos.top += 21)-19, "left" : pcPos.left+2 });
									});
								}
							});
							
							var blockUI = $("<div id='bgBlock' />");  //遮罩層
				
							$("body").append($(blockUI));
							$(blockUI)   
								.animate({
									"opacity": "0.4"     //改變遮罩的透明度
								}, 200, function() {						
									$("div#purchDetail").css({ "position": "absolute", "top": ($(document).height()/2) - 300 + "px", "left": ($(document).width()/2) - 250 + "px", "z-index": "99999" }).show("slow");
								})
								.css({
									"position": "absolute",
									"width": $(document).width(),    //與頁面同寬
									"height": $(document).height() + 60,  //與頁面同高,以蓋住整個文件內容
									"z-index": "9999",
									"background-color": "black",     //遮顏色:黑色
									"top": "0",
									"left": "0"
								});
														
							$("input#newPurchProd").button().click(function(event) {
								event.preventDefault();
								
								$("select#prodType").removeClass("ui-state-error");
								$("input#prodNum").removeClass("ui-state-error");
								$("p.validateTips", "table#DetailList").text("");
								
								var valid = true;
								valid = valid && checkEmpty($("select#prodType"), " [產品名稱] ");
								valid = valid && checkEmpty($("input#prodNum"), " [產品數量] ");
								valid = valid && checkNumber($("input#prodNum"),  /[0-9]/g, " [產品數量] ");
								
								if(valid)
								{
									var pcPos = $("div#productCart").offset();
									$("<div id='eachProduct' style='position:fixed; border:1px solid #336699; color:#FFFFCC; background-color:#336699; padding:2px; margin-bottom:1px; cursor:hand; z-index:999999;' val=" + $("select#prodType").val() + "_" + $("select#prodList").val() + ">" + $("select#prodList option:selected").text() + " / " + $("input#prodNum").val() + "</div>")
										.draggable({ 
											axis: "y",
											stop: function(event, ui) { 
												var $obj = $("div#productCart");
												var pcPos = $obj.offset();
												var pcMinTop = pcPos.top;
												var pcMaxTop = pcMinTop + 200;
												if((ui.offset.top < pcMinTop || ui.offset.top > pcMaxTop))
												{
													$(this).remove();
												}
												
												$obj.find("div#eachProduct").each(function() {
													$(this).css({ "top" : (pcPos.top += 21)-19, "left" : pcPos.left+2 });
												});
											} 
										})
										.appendTo($("div#productCart"));
										
										var $pcDivObj = $("div#productCart");
										var pcDivPos = $pcDivObj.offset();							
										$("div#productCart").find("div#eachProduct").each(function() {
											$(this).css({ "top" : (pcDivPos.top += 21)-19, "left" : pcPos.left+2 });
										});
								}
							});
							...
							$("select#prodType").change(function() {
								$.ajax({
									type: "GET",
									url: "products_info.php",
									data: "type=LISTNAME&typeid=" + $(this).val(),
									dataType: "json",
									success: function(data) {
										if(data)
										{
											...
											for(var i in data)
											{
												prodList += "<option value=" + data[i].prod_no + ">" + data[i].prod_name + "</option>";
											}
											$("select#prodList").html(prodList);
										}
									},
									error: function(data, textStatus) {
										alert("存取錯誤:" + textStatus + " - " + data);
							...
							$("input#insertPD").button().click(function(event) {
								event.preventDefault();
								
								var valList = [];
								$("div#productCart").find("div#eachProduct").each(function() {
									var idArr = $(this).attr("val").split("_");
									var eachNum = $("input#prodNum").val();
									valList.push({ purch_no: dataId, prod_no: idArr[1], purch_num: eachNum });
								});
								
								$.ajax({
									type: "POST",
									url: "purchase_info.php",
									data: "type=UPDDETAIL&id=" + dataId + "&val=" + valList,
									dataType: "json",
									cache: false,
									success: function(data) {
										if(data)
										{
											$.pagerComponent.Details.showInfo($link, dataId);
										}
									},
									error: function(data, textStatus) {
										alert("存取錯誤:" + textStatus + " - " + data);
								...
								$(blockUI).remove();
								$("div#purchDetail").remove();
							});
							
							$("input#cancelPD").button().click(function(event) {
								event.preventDefault();
		...
		EditForm: {
			loadForm: function(op, $table) {
				var $vendNo = $("select#vend_no"), $pDate = $("input#pDate");

				var inputFields = $([]).add($vendNo).add($pDate);
				...
				$.ajax({
					...
					success: function(data) {
						if(data)
						{
							...
							for(var i in data)
							{
								options += "<option value=" + data[i].vend_no + ">" + data[i].vend_name + "</option>";
							}
							$vendNo.append(options);
						}
					},
					error: function(data, textStatus) {
						alert("存取錯誤:" + textStatus + " - " + data);
				...
				if(op !== "NEW")   //修改時,先載入原來資料
				{
					$.ajax({
						...
						success: function(data) {
							if(data)
							{
								$vendNo.val(data[0].vend_no).prop("disabled", true);
								$pDate.val(data[0].purch_date);
							}
						},
						error: function(data, textStatus) {
							alert("存取錯誤:" + textStatus + " - " + data);
				...
				var $dialog = $("#dialog").dialog({
					...
					buttons: {
						"確定": function() {
							...
							isValid = isValid && checkEmpty($vendNo, "[ 供應商 ]");
							isValid = isValid && checkEmpty($pDate, "[ 交貨日期 ]");
							
							if(isValid == true)
							{
								if(op === "NEW")
									var inputData = { type: "NEW", vend_no: $vendNo.val(), purch_date: $pDate.val(), edate: Opts.eDate, sdate: Opts.sDate };
								else
									var inputData = { type: "SAVE", id: op, purch_date: $pDate.val(), edate: Opts.eDate, sdate: Opts.sDate };
								...
								$.ajax({
									...
									success: function(data) {
										...
										{
											alert("資料已送出!");
											$.pagerComponent.Datas.loadData($table, pageChange);
										}
									},
									error: function(data, textStatus) {
										alert("存取錯誤:" + textStatus + " - " + data);
		...
		Filter: {
			...
			initialize: function($table) {
				...
				$("table#searchTb").remove();
				
				//在資料表上方創建一列搜尋列
				$("<table id='searchTb' width='" + width + "' style='border:1px solid #DDD; margin-bottom:5px;'><tr id='filterBar'><td id='filterList' style='text-align:left;'><span id='filterTitle'>請先選擇欄位: </span><input type='button' id='submit' value='搜尋' /><input type='button' id='reset' value='清除' /></td></tr>").insertBefore($table);
		...
		Sort: {
			...
			initialize: function($table) {
				...
				$thElement.each(function(index) {
					...
					if(index > 0 && index < $thElement.size())
		...
		Datas: {
			...
			loadData: function($table, action) {
				var Opts = $.pagerComponent.options[0];
				var sql = "type=LIST&sdate=" + Opts.sDate + "&edate=" + Opts.eDate;
	...
	function checkNumber($obj, regexp, objName)
	{
		if(!regexp.test($obj.val()))
		{
			$obj.addClass("ui-state-error");
			tips(objName + "欄位必須輸入數字，請重新輸入!");
			return false;
		}
		else
			return true;
